...
describe('createMessage', () => {
  test('调用该方法应创建对应的 Message 组件', async () => {
    ...
    await rAF()
    expect(document.querySelector('.au-message')).toBeTruthy()
    instance.destory()
    await rAF()
    // console.log('销毁后 >>> HTML', document.body.innerHTML)
    expect(document.querySelector('.au-message')).toBeFalsy()
  })
  ...
  test('多次调用方法应创建多个实例对象', async () => {
    ...
    await rAF()
    const elements = document.querySelectorAll('.au-message')
